def get_affiliate_text_by_info(title, tag_ids):
    ...
    mbti_tag_0 = next((mbti for mbti, info in mbti_dict.items() if info["id"] == tag_ids[0]), None)
    mbti_tag_1 = next((mbti for mbti, info in mbti_dict.items() if info["id"] == tag_ids[1]), None)
